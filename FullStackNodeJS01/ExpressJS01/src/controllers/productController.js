...
const getProducts = async (req, res) => {
    try {
        ...
        if (result.EC === 0) {
            const p = result.DT.pagination;
            res.set({
                'X-Total-Count': p.totalItems,
                'X-Total-Pages': p.totalPages,
                'X-Current-Page': p.currentPage,
                'X-Per-Page': p.itemsPerPage,
                'X-Has-Next-Page': String(p.hasNextPage),
                'X-Next-Page': p.nextPage ? String(p.nextPage) : '',
            });
        }
        return res.status(result.EC === 0 ? 200 : 400).json(result);
    } catch (error) {
